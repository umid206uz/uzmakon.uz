...
$(document).ready(function ($){
    $('.field-orders-take_time').css('display', 'none');
});
$('#orders-status').change(function (){
    let _this = $(this);
    let status = _this.val();
    if (status == 5){
        $('.field-orders-take_time').css('display', 'block');
    }else{
        $('.field-orders-take_time').css('display', 'none');
    }
});
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <?= $form->field($model, 'region_id')->widget(Select2::classname(), [
                                'data' => ArrayHelper::map(Regions::find()->where(['parent_id' => 1])->andWhere(['!=', 'code', 1700])->all(), 'code', 'name'),
                                'hideSearch' => true,
                                'language' => 'uz',
                                'options' => [
