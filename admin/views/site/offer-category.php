...
/* @var $categoryItem common\models\Category */
...
$this->title = $categoryItem->metaTitleTranslate;
?>
<!--**********************************
    Content body start
***********************************-->
        ...
        <div class="card">
            <div class="card-body">
                <?php foreach ($category as $categoryOne):?>
                    <a href="<?=Url::to(['/site/offer-category', 'id' => $categoryOne->id])?>" class="btn mb-1 btn-outline-<?=($categoryOne->id == $categoryItem->id) ? "danger" : "primary"?>">
                        <?=$categoryOne->titleTranslate?> <span class="badge badge-<?=($categoryOne->id == $categoryItem->id) ? "danger" : "primary"?> ml-2"><?= Product::find()->where(['category_id' => $categoryOne->id, 'status' => 1])->count()?></span>
                    </a>
                <?php endforeach;?>
            </div>
        </div>
