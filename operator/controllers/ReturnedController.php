...
class ReturnedController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        $allowedRoles = ['operator_returned'];
    ...
    {
        $query = OrdersReturn::find()->where(['status' => OrdersReturn::STATUS_NEW, 'operator_id' => null])->orWhere(['status' => Orders::STATUS_NEW, 'operator_id' => Yii::$app->user->id])->orderBy(['operator_id' => SORT_DESC, 'id' => SORT_DESC]);
    ...
    {
        $result = $this->findOrdersByStatus(OrdersReturn::STATUS_NEW);
    ...
    public function actionOrderAjax()
    {
        ...
        $check = OrdersReturn::find()->where(['operator_id' => Yii::$app->user->id, 'status' => OrdersReturn::STATUS_NEW])->count();
        if($check > 4){
            return [
                'status' => 'success',
                'control' => 2
            ...
            if (Yii::$app->request->get('order_id')){
                ...
                $model = OrdersReturn::findOne($order_id);
                if ($model->operator_id == null){
                    ...
                    return [
                        ...
                        'phone_mask' => Yii::$app->formatter->asPhone($model->customer_phone),
                        'control' => 1
                    ];
                }else{
                    return [
    ...
    {
        $model = new UpdateReturnedForm();
        $order = OrdersReturn::findOne($id);
        $model->loadOrder($order);

        if ($model->load(Yii::$app->request->post()) && $model->validate()) {
            if ($model->orderUpdate($order)) {
                Yii::$app->session->setFlash('success', Yii::t("app","Status changed"));
